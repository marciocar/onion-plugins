...
Para orquestração paralela de múltiplos especialistas, use a ferramenta **Workflow** nativa do Claude Code (fan-out/fan-in) — ver a skill `onion-orchestration` e o comando `/onion:orchestrate`. Não há MCP de orquestração.
...
> 📖 Árvore completa: KB **c4-adr-patterns.md §1**.
