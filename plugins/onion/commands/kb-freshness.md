...
# /onion:kb-freshness — Auditoria de Frescor de Knowledge Bases
...
- Em auditorias periódicas de saúde documental (ex.: junto com `/onion-product:docs-health`).
...
5. **Grafo primeiro**: achados `HISTORICAL` e alertas transversais são achados estruturados —
   materialize via `/onion:kg` (nó `claim`, `layer: audit`, `trace` à KB) **antes** de fechar o
...
⚠  Item #1 (lineup de modelos) falhou em 4 KBs — revisão em lote recomendada.
   Sugestão: /onion:orchestrate substituir referências a modelos obsoletos em lote

─── PRÓXIMOS PASSOS ───────────────────────────
1. Arquivar/reescrever KBs HISTORICAL (2 arquivos)
2. Atualizar KBs STALE em lote via /onion:orchestrate
3. Rodar /onion-product:build-index após os refreshes
4. Re-executar /onion:kb-freshness para confirmar frescor
...
/onion:kb-freshness docs/knowledge-base/concepts/ai-agent-design-patterns.md

# Auditar um diretório inteiro
/onion:kb-freshness docs/knowledge-base/frameworks/

# Auditar usando glob pattern
/onion:kb-freshness docs/knowledge-base/concepts/*.md
```
...
- Este comando **nunca cria nem modifica KBs** — apenas audita e relata. Para
  ...
  existente) ou `/onion:orchestrate` (atualização em lote de muitas KBs de uma vez).
...
- Saúde documental ampla: `/onion-product:docs-health`
- Atualização em lote: `/onion:orchestrate`
- Índice mestre: `docs/knowledge-base/index.md` (atualizar via `/onion-product:build-index`)
