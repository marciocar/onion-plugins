...
# /onion:kg — Investigação como Knowledge Graph SDAAL
...
```bash
/onion:kg novo auditoria-seguranca          # cria docs/onion/graph/auditoria-seguranca.kg.yaml
/onion:kg docs/onion/graph/onion-evolution-2026-07.kg.yaml   # modela/atualiza e roda radar
/onion:kg                                    # localiza o mais recente e roda o radar
/onion:kg map command-center                 # PFR F0-F4: inventário → atom-map → .kg.yaml → radar (software)
/onion:kg diagnose cliente-acme-2026-07       # engajamento como KG 2-camadas; radar = diagnóstico (atenção/gargalo/reconciliação)
/onion:kg narrate m2-bridge-logto-2026-07     # autora a narração pré-cozida (tour + resumos) p/ o console
```

## ⚠️ Notas

- **Grafo primeiro, relatório depois** (contrato para os consumidores): `meta:evolve`,
  `/onion:kb-freshness` e `/onion:context-freshness` — e qualquer comando que produza achados
