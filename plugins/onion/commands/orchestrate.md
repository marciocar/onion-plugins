...
```
/onion:orchestrate <tarefa a paralelizar>
```
...
1. **Persista a síntese no repo** — `docs/**/research/*.md` (ou local durável apropriado). **Nunca**
   ...
   nunca o vê.
2. **Materialize/atualize o `.kg.yaml`** dos achados/decisões via `/onion:kg` e valide com
   `bash ${CLAUDE_PLUGIN_ROOT}/validation/kg-radar.sh <arquivo>` (**exit 0**).

Fecha o ciclo `read(KG)→verify→act→write(KG)`
([knowledge-graph-sdaal.md](${CLAUDE_PLUGIN_ROOT}/kb/knowledge-graph-sdaal.md)
...
/onion:orchestrate auditar conformidade de todos os agentes contra as meta-specs

# Migração mecânica multi-arquivo (mutação partição-primeiro → 1 branch; ver script no Passo 3)
/onion:orchestrate adicionar o campo `version` ao frontmatter de todos os comandos sem versão

# Pesquisa fan-out citada (fan-out-and-synthesize)
/onion:orchestrate pesquisar e comparar 5 fontes sobre padrões de orquestração de agentes em 2026
```
