...
Gerencia o workflow de **code review automático** no CI (GitHub Actions) do projeto. É uma tarefa de **configuração de integração** (não de GitFlow) — por isso vive em `meta/`, ao lado de `/onion:setup-integration`.
...
Para postar o resultado da validação como comentário no PR, usar o **adapter forge** (`forge.addReviewComment(...)` em ${CLAUDE_PLUGIN_ROOT}/utils/forge/) — **não** chamar `gh`/API direto (integrations.md §9).
...
- Integração / secrets: .env.example · `/onion:setup-integration`
