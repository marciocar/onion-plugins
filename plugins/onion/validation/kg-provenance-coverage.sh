...
#     · Diário (docs/onion/diary/**, /onion:diary) — autobiografia com forcing
...
#   é o que alimenta o modo `/onion:kg backfill`.
#
...
# Sai SEMPRE 0: é diagnóstico, não gate. Quem consome isto é o /onion:kg backfill,
...
if [ "${BASELINE_PRESENT}" -eq 0 ]; then
  ...
  while IFS= read -r p; do
    [ -n "${p}" ] || continue
    say "HARD" "NEW" "${p}" "documento de análise NOVO sem nó no grafo — nenhum .kg.yaml o cita em trace:/evidence:. Achado estruturado nasce no grafo; o markdown é vista. Modele com /onion:kg e valide: bash ${CLAUDE_PLUGIN_ROOT}/validation/kg-radar.sh <grafo>.kg.yaml"
  done < "${TMP}/novos"
